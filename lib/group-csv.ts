export type GroupInput={name:string;url:string;platform:string;visibility:string;audience_notes:string|null;posting_rules:string|null;approval_required:boolean;product_names:string[]};
...
export function normalizeGroupUrl(raw:string){
 ...
 const linkedin=['linkedin.com','www.linkedin.com'].includes(url.hostname.toLowerCase());
 if(!linkedin&&!['facebook.com','www.facebook.com','m.facebook.com','mbasic.facebook.com','web.facebook.com'].includes(url.hostname.toLowerCase())||url.username||url.password||url.port)throw new Error('Facebook বা LinkedIn গ্রুপের সরাসরি লিংক দাও');
 ...
 if(linkedin&&!/^\d+$/.test(id))throw new Error('LinkedIn গ্রুপের সংখ্যার ID থাকতে হবে');
 return 'https://www.'+(linkedin?'linkedin.com':'facebook.com')+'/groups/'+id+'/';
}
...
export function readGroupCsv(text:string,existingUrls:string[],defaults:{visibility?:string;approval_required?:boolean}={}):PreviewRow[]{
 ...
 const aliases:Record<string,string[]>={platform:['platform'],visibility:['visibility','privacy','group_visibility'],url:['url','link','group_url','group_link'],name:['name','group_name'],products:['products','product','services','service','product_service','product_or_service'],audience_notes:['audience_notes','audience'],posting_rules:['posting_rules','rules'],approval_required:['approval_required','approval']};
 ...
 const platform=new URL(url).hostname==='www.linkedin.com'?'linkedin':'facebook';
 if(hasHeader&&field('platform')&&field('platform').toLowerCase()!==platform)throw new Error('platform কলাম ও লিংকের প্ল্যাটফর্ম মিলছে না');
 const visibility=(hasHeader?field('visibility').toLowerCase():'')||defaults.visibility||'unknown';
 if(!['public','private','unknown'].includes(visibility))throw new Error('visibility: public, private বা unknown দাও');
 const name=(hasHeader?field('name'):'')||(platform==='linkedin'?'LinkedIn':'Facebook')+' group · '+url.split('/')[4];
 ...
 const group={name,url,platform,visibility,product_names,audience_notes:hasHeader?field('audience_notes')||null:null,posting_rules:hasHeader?field('posting_rules')||null:null,approval_required:approval?['true','yes','1','হ্যাঁ'].includes(approval):(defaults.approval_required??false)};
