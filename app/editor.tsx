...
export default function Editor({table,row,initial={},data,close,save,refresh}:Props){
 ...
 if(table==='contents')fields=['business_id','product_id','title','topic','offer_name','objective','format','content_pillar','target_audience','hook','cta','brief','script','source','ai_notes','status','tags'];
 ...
 if(table==='destinations'){if(v.destination_type==='group'){payload.visibility=v.visibility;payload.url=normalizeGroupUrl(String(payload.url||''));const detected=new URL(payload.url).hostname==='www.linkedin.com'?'linkedin':'facebook';if(detected!==payload.platform)throw new Error('গ্রুপের লিংক ও নির্বাচিত প্ল্যাটফর্ম মিলছে না।')}if(v.destination_type==='group'&&!payload.url)throw new Error('ফেসবুক গ্রুপের লিংক দাও।');if(payload.url&&!/^https?:\/\//i.test(payload.url))throw new Error('লিংক https:// দিয়ে শুরু হতে হবে।')}
 if(table==='posts'){
