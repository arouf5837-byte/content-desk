...
export default function Workspace({session}:{session:Session}){
 ...
 const posts=view==='posts'?allFilteredPosts.filter(p=>{if(postScope==='today')return isTodayDhaka(postDate(p));if(postScope==='recent')return isWithinDaysDhaka(postDate(p),7);return true;}):allFilteredPosts;
 ...
 async function deleteAllContent(){
  ...
  try{
   ...
   setNotice('সব কনটেন্ট, সংস্করণ ও পোস্ট সফলভাবে মুছে ফেলা হয়েছে!');
 ...
 function contentTable(rows:Row[],preview=false){
  ...
  return slice.length?<><Table><TableHeader><TableRow><TableHead>কনটেন্ট</TableHead><TableHead>বিজনেস</TableHead><TableHead>প্রোডাক্ট / অফার</TableHead><TableHead>লিড ম্যাগনেট / প্ল্যাটফর্ম</TableHead><TableHead>অবস্থা</TableHead><TableHead>পোস্ট সম্পন্ন</TableHead><TableHead>তারিখ</TableHead><TableHead>অ্যাকশন</TableHead></TableRow></TableHeader><TableBody>{slice.map(c=>{const variants=data.content_variants.filter(v=>v.content_id===c.id);const linked=data.posts.filter(p=>variants.some(v=>v.id===p.variant_id));const isPosted=linked.some(p=>p.status==='published');const lm=leadMagnetOf(c.lead_magnet_id);const prod=(data.products||[]).find(p=>p.id===c.product_id);const Icon=c.format==='video'?Video:c.format==='text'?FileText:ImageIcon;return <TableRow key={c.id}><TableCell><button className="content-title" onClick={()=>setDetail(c.id)}><span className={'format-icon format-'+c.format}><Icon size={19}/></span><div><strong>{c.title}</strong><small>{formats[c.format]}{c.topic?' · '+c.topic:''}</small></div></button></TableCell><TableCell><span className="business-tag">{bname(c.business_id)}</span></TableCell><TableCell className="muted">{prod?.name||c.offer_name||'—'}</TableCell><TableCell>{lm?<span className="keyword-badge" title={lm.name}><Gift size={12}/>{c.cta_keyword||lm.cta_keyword}</span>:<div className="platform-stack">{[...new Set(variants.map(v=>v.platform))].map(p=><span key={p} title={platforms[p]}><Platform value={p}/></span>)}{!variants.length&&<span className="muted">—</span>}</div>}</TableCell><TableCell><Badge status={c.status}/></TableCell><TableCell>{isPosted?<button className="posted-pill active" title="ক্লিক করলে পোস্টটি খসড়ায় ফিরে যাবে" onClick={()=>markAsPosted(c)} disabled={busyAction}><CheckCheck size={14}/> পোস্ট হয়েছে</button>:<button className="mark-posted-btn" title="১-ক্লিকে পোস্ট সম্পন্ন মার্ক করুন" onClick={()=>markAsPosted(c)} disabled={busyAction}><Check size={14}/> পোস্ট সম্পন্ন</button>}</TableCell><TableCell className="tabular">{dateLabel(c.created_at)}</TableCell><TableCell><div className="row-actions"><button className="icon-button" title="সম্পাদনা" aria-label="সম্পাদনা" onClick={()=>setEditing({table:'contents',row:c})}><Pencil size={16}/></button><button className="icon-button" aria-label={c.title+' খোলো'} onClick={()=>setDetail(c.id)}><ArrowUpRight size={18}/></button><button className="icon-button danger" aria-label={c.title+' মুছুন'} title="কনটেন্ট মুছে ফেলুন" onClick={()=>setConfirmDeleteContent(c)}><Trash2 size={16}/></button></div></TableCell></TableRow>})}</TableBody></Table>{!preview&&pagination(rows.length)}</>:<Blank title={f.dateMode==='single'?`${formatBanglaDay(f.selectedDate)}-এ কোনো কনটেন্ট নেই`:(data.contents.length?'এই ফিল্টারে কনটেন্ট নেই':'তোমার প্রথম কনটেন্ট যোগ করো')} description={f.dateMode==='single'?'এই দিনে নতুন আইডিয়া, পোস্ট বা স্ক্রিপ্ট যোগ করো।':'আইডিয়া, লেখা, ছবি কিংবা ভিডিও—এখান থেকেই শুরু।'}><button className="primary" onClick={()=>newRecord('contents')} disabled={!activeBusinesses.length}><Plus size={16}/> কনটেন্ট যোগ করো</button></Blank>;
 }
 ...
 {['overview','library','posts','analytics'].includes(view)&&<div className="daily-bar"><div className="daily-nav"><button className="icon-button" onClick={()=>shiftDay(-1)} title="আগের দিন" aria-label="আগের দিন"><ChevronLeft size={18}/></button><div className="daily-display"><CalendarDays size={18}/><span>{formatBanglaDay(f.selectedDate)}</span><input type="date" className="daily-picker" value={f.selectedDate} onChange={e=>e.target.value&&update('selectedDate',e.target.value)} title="ক্যালেন্ডার থেকে দিন নির্বাচন করো" aria-label="তারিখ নির্বাচন"/></div><button className="icon-button" onClick={()=>shiftDay(1)} title="পরের দিন" aria-label="পরের দিন"><ChevronRight size={18}/></button>{f.selectedDate!==today&&<button className="secondary small-btn" onClick={()=>update('selectedDate',today)}>আজকে ফিরুন</button>}</div><div className="daily-stats"><span className="day-stat-chip"><span className="dot dot-green"/> কনটেন্ট: <strong>{number(contents.length)}</strong></span><span className="day-stat-chip"><span className="dot dot-blue"/> পোস্ট সম্পন্ন: <strong>{number(published.length)}</strong></span><span className="day-stat-chip"><span className="dot dot-amber"/> বাকি: <strong>{number(pending.length)}</strong></span></div><div className="daily-mode-switch"><button className={'mode-btn '+(f.dateMode==='single'?'active':'')} onClick={()=>update('dateMode','single')} title="শুধুমাত্র নির্বাচিত দিনের কনটেন্ট দেখাবে">একক দিন (Single Day)</button><button className={'mode-btn '+(f.dateMode==='all'?'active':'')} onClick={()=>update('dateMode','all')} title="সব দিনের কনটেন্ট একসাথে দেখাবে">সব দিন (All)</button></div></div>}
 ...
 {view!=='settings'&&view!=='products'&&view!=='groups'&&view!=='calendar'&&<section className="filter-panel"><div className="filter-row"><div className="search-box"><Search size={17}/><input aria-label="খোঁজো" placeholder={view==='crm'?'নাম, হ্যান্ডেল, ইমেইল বা নোট খোঁজো...':'কনটেন্ট, বিষয় বা ট্যাগ খোঁজো...'} value={f.q} onChange={e=>update('q',e.target.value)}/>{f.q&&<button onClick={()=>update('q','')} aria-label="সার্চ মুছো"><X size={15}/></button>}</div><Pick label="বিজনেস ধরন" value={f.businessKind} onChange={v=>update('businessKind',v)} options={businessKinds}/>{view!=='groups'&&view!=='crm'&&<Pick label="প্রোডাক্ট / সার্ভিস" value={f.product} onChange={v=>update("product",v)} options={{all:"সব প্রোডাক্ট / সার্ভিস",none:"প্রোডাক্ট ছাড়া",...Object.fromEntries((data.products||[]).filter(p=>activeBusinesses.some(b=>b.id===p.business_id)&&(f.business==="all"||p.business_id===f.business)).map(p=>[p.id,p.name+(p.archived_at?" · আর্কাইভ":"")]))}}/>}{view!=='lead_magnets'&&<Pick label="প্ল্যাটফর্ম" value={f.platform} onChange={v=>update('platform',v)} options={{all:'সব প্ল্যাটফর্ম',...platforms}}/>}<Pick label="অবস্থা" value={f.status} onChange={v=>update('status',v)} options={{all:'সব অবস্থা',...(view==='library'?contentStates:view==='lead_magnets'?leadMagnetStates:view==='crm'?leadStatuses:postStates)}}/>{view!=='lead_magnets'&&view!=='crm'&&<button className={'secondary filter-toggle '+(expanded?'selected':'')} onClick={()=>setExpanded(!expanded)}><SlidersHorizontal size={16}/>ফিল্টার {activeFilters>0&&<span className="filter-count">{number(activeFilters)}</span>}</button>}{activeFilters>0&&<button className="text-button" onClick={()=>setF({...defaults,business:f.business,selectedDate:f.selectedDate,dateMode:f.dateMode})}>মুছে ফেলো</button>}</div>{expanded&&view!=='lead_magnets'&&view!=='crm'&&<div className="advanced-filters"><label>ফরম্যাট<Pick label="ফরম্যাট" value={f.format} onChange={v=>update('format',v)} options={{all:'সব ফরম্যাট',...formats}}/></label><label>তৈরির মাধ্যম<Pick label="তৈরির মাধ্যম" value={f.source} onChange={v=>update('source',v)} options={{all:'সব মাধ্যম',...sources}}/></label><label>অ্যাকাউন্ট / গ্রুপ<Pick label="অ্যাকাউন্ট" value={f.destination} onChange={v=>update('destination',v)} options={{all:'সব অ্যাকাউন্ট',...Object.fromEntries(data.destinations.filter(d=>activeBusinesses.some(b=>b.id===d.business_id)&&(f.business==='all'||d.business_id===f.business)).map(d=>[d.id,d.name]))}}/></label>{view!=='library'&&<label>বিতরণ<Pick label="বিতরণ" value={f.distribution} onChange={v=>update('distribution',v)} options={{all:'সব ধরনের',organic:'অর্গানিক',paid:'পেইড',mixed:'মিশ্র'}}/></label>}<label>শুরুর তারিখ<input type="date" value={f.from} onChange={e=>update('from',e.target.value)}/></label><label>শেষের তারিখ<input type="date" min={f.from} value={f.to} onChange={e=>update('to',e.target.value)}/></label><p className="filter-hint">{view==='library'?'কনটেন্ট যোগ করার তারিখ অনুযায়ী।':'প্রকাশের তারিখ; বাকি পোস্টের ক্ষেত্রে নির্ধারিত বা তৈরির তারিখ অনুযায়ী।'} সব সময় ঢাকা।</p></div>}</section>}
 ...
 <AlertDialog open={confirmDeleteAll} onOpenChange={setConfirmDeleteAll}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>সব কনটেন্ট মুছে ফেলতে চাও?</AlertDialogTitle><AlertDialogDescription>তোমার বিজনেসের সকল কনটেন্ট, সংস্করণ, নির্ধারিত পোস্ট এবং পরিসংখ্যান চিরতরে মুছে যাবে। এটি আর ফিরিয়ে আনা যাবে না।</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={busyAction}>বাতিল</AlertDialogCancel><AlertDialogAction disabled={busyAction} className="danger-btn" onClick={e=>{e.preventDefault();void deleteAllContent()}}>{busyAction?'মুছে ফেলা হচ্ছে…':'সব কনটেন্ট মুছুন'}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
